				   ...
				   const TCHAR *szDelim /*= NULL */ )
{
	...
	__MY_TRACE(_T("CAnnNetwork::parseNetLayers() Enter.\n"));
				   ...
				   const TCHAR *szDelim /*= NULL */ )
{
	...
	__MY_TRACE(_T("CAnnNetwork::parseTransFcns() Enter.\n"));
	...
	while (pszOffset <= pszEndOf) {
		...
		if (lenFcnName > 0) {
			if (pNetLayers != NULL) {
				...
				__MY_TRACE(_T("transFcn[%d] = %s. (nRetSize = %d)\n"), _indexFcns, szFcnName, nRetSize);
			}
			_indexFcns++;
		}
		if (pszDest != NULL)
		  ...
		  bool bInitWeightsNow /*= FALSE */ )
{
	...
	__MY_TRACE(_T("trimString(\"%s\") = %s. (nRetSize = %d)\n"), szText, szBuffer, nRetSize);
