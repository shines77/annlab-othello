...
//  inline MatrixT<T>& operator =  (int _value              );
    ...
    /*
    friend inline FastMatrix<T> operator + (value_type _value, FastMatrix<T>& _Right);
    friend inline FastMatrix<T> operator - (value_type _value, FastMatrix<T>& _Right);
    friend inline FastMatrix<T> operator * (value_type _value, FastMatrix<T>& _Right);
    friend inline FastMatrix<T> operator / (value_type _value, FastMatrix<T>& _Right);

    friend inline ostream& operator << (ostream& out, const FastMatrix<T>& _matrix);
    friend inline istream& operator >> (istream& in, FastMatrix<T>& _matrix);
    //*/
    ...
    inline void fill     (int _initFcn = INIT_FCN_DEFAULT,
                        value_type _fillVal = value_type(0.0));
    inline int  refill   (int _initFcn, value_type _fillVal = value_type(0.0));
    ...
    inline int ones   (void);
    inline int rands  (void);
    inline int rands2 (void);
    ...
    inline int order  (int _rows, int _cols);
    ...
    static inline FastMatrix<T> _order  (int _rows, int _cols);
...
protected:
    ...
    inline void init(int _rows, int _cols, int _initFcn = INIT_FCN_DEFAULT,
            value_type _fillVal = value_type(0.0));
    inline void init_ex(const TCHAR *szName, int _rows, int _cols,
