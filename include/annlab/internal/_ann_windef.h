...
#if !defined(_ANN_ann_windef_H_) || defined(_ANN_INTERNAL_ANN_WINDEF_H_)
#error Do not #include this internal file directly; use public ANNLAB headers instead.
#endif /* _ANN_ann_windef_H_ */

#ifndef _ANN_INTERNAL_ANN_WINDEF_H_
#define _ANN_INTERNAL_ANN_WINDEF_H_
...
#    if TBB_USE_DEBUG 
#        if !defined(_DEBUG)
#            pragma message(__FILE__ "(" __ANN_STRING(__LINE__) ") : Warning: Recommend using /MDd if compiling with TBB_USE_DEBUG!=0")
#        endif
#    else
#        if defined(_DEBUG)
#            pragma message(__FILE__ "(" __ANN_STRING(__LINE__) ") : Warning: Recommend using /MD if compiling with TBB_USE_DEBUG==0")
#        endif
#    endif
#endif

#if __ANN_BUILD && !defined(__ANN_NO_IMPLICIT_LINKAGE)
#define __ANN_NO_IMPLICIT_LINKAGE   1
#endif

#if _MSC_VER
    #if !__ANN_NO_IMPLICIT_LINKAGE
        #ifdef __ANN_LIB_NAME
	        #pragma comment(lib, __ANN_STRING(__ANN_LIB_NAME))
        #else
			#ifdef _DEBUG
				#pragma comment(lib, "ann_debug.lib")
			#else
...
#endif  // _ANN_INTERNAL_ANN_WINDEF_H_
